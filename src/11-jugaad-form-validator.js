...
export function validateForm(formData) {
  ...
  if (typeof email !== "string" || email.indexOf("@") === -1 || email.indexOf("@") !== email.lastIndexOf("@") || email.indexOf(".") === -1 || email.indexOf(".") < email.indexOf("@")) {
    errors.email = "Invalid email format";
  }
  if (typeof phone !== "string" || phone.length !== 10 || !["6", "7", "8", "9"].includes(phone[0]) || ![...phone].every(ch => ch >= "0" && ch <= "9")) {
    errors.phone = "Invalid Indian phone number";
  }

  if (typeof age !== "number" || age < 16 || age > 100 || !Number.isInteger(age)) {
    errors.age = "Age must be an integer between 16 and 100";
  }

  if (typeof pincode !== "string" || pincode.length !== 6 || pincode.startsWith("0") || ![...pincode].every(ch => ch >= "0" && ch <= "9")) {
